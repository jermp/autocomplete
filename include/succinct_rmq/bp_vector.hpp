...
public:
    ...
    size_t bytes() const {
        return rs_bit_vector::bytes() +
               essentials::pod_bytes(m_internal_nodes) +
               essentials::vec_bytes(m_block_excess_min) +
               essentials::vec_bytes(m_superblock_excess_min);
    }

    template <typename Visitor>
    void visit(Visitor& visitor) {
        rs_bit_vector::visit(visitor);
        visitor.visit(m_internal_nodes);
        visitor.visit(m_block_excess_min);
        visitor.visit(m_superblock_excess_min);
    }
...
protected:
    static const size_t bp_block_size =
        4;  // to increase confusion, bp block_size is not necessarily
            // rs_bit_vector block_size
    ...
    std::vector<block_min_excess_t> m_block_excess_min;
    std::vector<excess_t> m_superblock_excess_min;
};
}  // namespace autocomplete
