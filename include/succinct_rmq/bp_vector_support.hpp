    ...
    excess_tables() {
        for (int c = 0; c < 256; ++c) {
            ...
            for (uint8_t i = 0; i < 8; ++i) {
                if ((c << i) & 128) {  // opening
                    ++excess;
                    if (excess > 0 && m_bwd_pos[c][(uint8_t)excess] ==
                                          0) {  // not already found
...
inline bool find_close_in_word(uint64_t word, uint64_t byte_counts,
    ...
    const uint64_t cum_exc_step_8 =
        (uint64_t(cur_exc) + ((2 * byte_counts - 8 * util::ones_step_8) << 8)) *
        util::ones_step_8;
    ...
    const uint64_t has_result =
        util::leq_step_8(cum_exc_step_8, min_exc_step_8);
...
inline bool find_open_in_word(uint64_t word, uint64_t byte_counts,
    ...
    const uint64_t cum_exc_step_8 =
        (uint64_t(cur_exc) -
         ((2 * rev_byte_counts - 8 * util::ones_step_8) << 8)) *
        util::ones_step_8;
    ...
    const uint64_t has_result =
        util::leq_step_8(cum_exc_step_8, max_exc_step_8);
...
uint64_t bp_vector::find_close(uint64_t pos) const {
    ...
    uint64_t local_rank =
        util::bytes_sum(byte_counts) - shift;  // subtract back the padding
    excess_t local_excess =
        static_cast<excess_t>((2 * local_rank) - (64 - shift));
    if (find_close_in_block(block_offset, local_excess + 1, sub_block + 1,
                            ret)) {
        return ret;
    }
...
uint64_t bp_vector::find_open(uint64_t pos) const {
    ...
    uint64_t sub_block = word_pos % bp_block_size;
    uint64_t local_rank =
        util::bytes_sum(byte_counts);  // no need to subtract the padding
    ...
    // Search in the found block
    bool found =
        find_open_in_block(found_block_offset, found_block_excess - pos_excess,
                           bp_block_size, ret);
    assert(found);
    (void)found;
    return ret;
    ...
    if (block_a == block_b) {
        ...
        if (superblock_a == superblock_b) {
            ...
            if (superblock_min_exc < min_exc) {
                excess_rmq_in_superblock(
                    superblock_min_idx * superblock_size,
                    (superblock_min_idx + 1) * superblock_size, block_min_exc,
                    block_min_idx);
            }
        ...
        if (block_min_exc < min_exc) {
            ...
            excess_rmq_in_block(block_min_idx * bp_block_size,
                                (block_min_idx + 1) * bp_block_size, cur_exc,
                                min_exc, min_exc_idx);
        ...
        excess_rmq_in_block(block_b * bp_block_size, word_b_idx, cur_exc,
                            min_exc, min_exc_idx);
    }
...
void bp_vector::build_min_tree() {
    ...
    for (uint64_t sub_block = 0; sub_block < m_bits.size(); ++sub_block) {
        if (sub_block % bp_block_size == 0) {
            ...
            if (sub_block) {
                assert(cur_block_min >=
                       std::numeric_limits<block_min_excess_t>::min());
                assert(cur_block_min <=
                       std::numeric_limits<block_min_excess_t>::max());
    ...
    while (n_complete_leaves < n_superblocks)
        n_complete_leaves <<=
            1;  // XXX(ot): I'm sure this can be done with util::msb...
    ...
    for (size_t superblock = 0; superblock < n_superblocks; ++superblock) {
        ...
        excess_t superblock_excess =
            get_block_excess(superblock * superblock_size);

        for (size_t block = superblock * superblock_size;
             block < std::min((superblock + 1) * superblock_size, n_blocks);
             ++block) {
            cur_super_min = std::min(
                cur_super_min, superblock_excess + block_excess_min[block]);
        }
    ...
    m_block_excess_min.swap(block_excess_min);
    m_superblock_excess_min.swap(superblock_excess_min);
}
}  // namespace autocomplete
